...
namespace GameServerExample2B
{
    public class GameServer
    {
        ...
        public GameClient GetClient(EndPoint e)
        {
            return clientsTable[e];
        }

        public int GetSpecifedObjectCount(uint e)
        {
            int count = 0;
            foreach (KeyValuePair<uint, GameObject> item in gameObjectsTable)
            {
                if (item.Value.ObjectType == e)
                    count++;
            }
            return count;
        }

        public GameObject GetSpecifedObject(uint e)
        {
            foreach (KeyValuePair<uint, GameObject> item in gameObjectsTable)
            {
                if (item.Value.ObjectType == e)
        ...
        private void Ack(byte[] data, EndPoint sender)
        {
            ...
            Console.WriteLine("sono entrato nell'ack");
        ...
        public GameObject GetObj(uint id)
        {
            float[] pos = new float[3];
            GameObject obj = gameObjectsTable[id];
            return obj;
        }
